...
def generate_random_locations(num_points, bounds, min_distance, min_home_radius = 5, max_trees_near_home = 3):
    """ Generate trees at random locations, ensuring they are at least min_distance apart.
        The area close to home (within min_home_radius) should be free. 
        Within near_home_dist there can only be a limited number of trees (max_trees_near_home)."""
    ...
    while len(points) < num_points and i < 10000:  # Limit iterations to avoid infinite loop
        ...
        if is_far_enough(x, y, points, min_distance) and dist_home >= min_home_radius and n_trees_near_home < max_trees_near_home:
            points.append((x, y, z))
            if dist_home < near_home_dist:
                n_trees_near_home += 1
        i += 1
...
def main(envparams, usdpath, props, home_pole=None, simulation_app=None, my_world = None, plot_graphs = False):
    ...
    num_locations, area_bounds, min_separation = envparams 
    ...
    locations = generate_random_locations(num_locations, full_area_bounds, min_separation)
    ...
    scene_name = "./maps/forest_" + str(num_locations) + "_trees_" + str(area_bounds) + "x" + str(area_bounds) +"_area"
    ...
    # add an underscore and a number to the name:
    ...
    # Save the locations so that this can be re-used for plotting landmarks later.
